<template>
  <div id="app" class="justify-center align-center">
    <form v-if="!showSignUpWindow" class="signup-form mt-10 mb-5">
...
<script setup lang="ts">
  ...
  const currRouteLocation = useRoute()
...
/**
 * This function authenticates users logging in using their Google account.
 * It checks if the user exists in the db and then redirects them to the home page.
 * If the user does not exist, it persists the user metadata in the db then redirects
  ...
  try{
    ...
    for (let i=0;i<existingClientEmails.length;i++){
      if(existingClientEmails.includes(encodedEmail)){
        ...
        username.value = currentClient.fname
...
/**
 * This function allows for the creation of new users signing up manually with
 * their email,username, and given passcode.
 * Leverages Email/Password Google auth. 
 */
    ...
    .then(async (userCredentials) => {
      ...
      for (let i=0;i<existingClientEmails.length;i++){
        if(existingClientEmails.includes(encodedEmail)){
          ...
          username.value = newClientUsername.value
...
/**
 * This function authenticates users logging in to the app by checking
 * the given email and passcode using Email/Password Google Auth.
 */
